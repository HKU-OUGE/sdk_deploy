/**
 * @file rl_gap_control_state.hpp
 * @brief rl policy running state for quadruped-wheel robot (Gap mode, sensor-based)
 */
#pragma once
...
namespace qw {
    class RLGapControlState : public StateBase {
    private:
    ...
    public:
        RLGapControlState(const RobotName &robot_name, const std::string &state_name,
            ...
            if (robot_name_ == RobotName::M20) {
                ...
                auto model_path = fs::canonical(base / ".." / ".." / "policy" / "gap_unified_policy.onnx");
                m20_policy_ = std::make_shared<M20SensorPolicyRunner>("m20_gap_policy", model_path.string());
            }
        ...
        virtual void OnEnter() {
            ...
            std::cout << "[RLGap] 开始网络预热 (Network Warm-up 150 frames)..." << std::endl;
            ...
            std::cout << "[RLGap] 网络预热完成！隐藏状态已收敛。" << std::endl;

            run_policy_thread_ = std::thread(std::bind(&RLGapControlState::PolicyRunner, this));
            StateBase::msfb_.UpdateCurrentState(RobotMotionState::RLGapControlMode);
        };
        ...
        virtual StateName GetNextStateName() {
            ...
            return StateName::kRLGapControl;
        }
    };
};
